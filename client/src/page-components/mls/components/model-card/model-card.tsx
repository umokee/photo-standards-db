...
import { GroupDetail, Metric, MlModel, TrainingStatus } from "@/types/contracts";
...
export const ModelCard = ({ model, group, expanded, onToggle }: Props) => {
  ...
  const trainingStageLabel =
    status === "pending"
      ? "Ожидает запуска"
      : (model.training_stage ?? trainingStatusLabel(status));
  ...
  const statusMessage =
    status === "failed"
      ? (model.training_error ?? "Ошибка обучения")
      : isTraining
        ? trainingStageLabel
        : null;
    ...
    <article className={clsx(s.root, expanded && s.expanded)}>
      ...
      {expanded && (
        <div className={s.body}>
          <ModelCardDetail model={model} group={group} />
        </div>
      )}
    </article>
  );
...
const ModelCardDetail = ({ model }: DetailProps) => {
  const isFailed = isTrainingFailedModel(model);
