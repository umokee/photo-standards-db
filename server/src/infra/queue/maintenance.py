...
from modules.tasks.service import cleanup_old_terminal_tasks


@procrastinate_app.periodic(cron="*/1 * * * *")
@procrastinate_app.task(
    queue="cpu",
    queueing_lock="retry_stalled_gpu_jobs",
    pass_context=True,
)
async def retry_stalled_gpu_jobs(context, timestamp) -> None:
...
@procrastinate_app.task(
    queue="cpu",
    queueing_lock="cleanup_old_tasks",
    pass_context=True,
)
async def cleanup_old_tasks(context, timestamp) -> None:
