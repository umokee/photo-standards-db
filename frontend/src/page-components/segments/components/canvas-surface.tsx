...
export default function Canvas({
  ...
  const { containerRef, size, imageRect, toImage, toCanvas, clampToImage } =
  ...
  const handleStageClick = (e: any) => {
    ...
    if (panConsumedClick.current) { panConsumedClick.current = false; return; }
  ...
  const handleGroupDragStart = (e: any) => {
    if (isSpaceDown.current) { e.target.stopDrag(); return; }
    isDragging.current = true;
    setEdgeGhost(null);
  };
